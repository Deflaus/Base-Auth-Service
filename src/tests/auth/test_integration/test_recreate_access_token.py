...
@pytest.mark.asyncio
async def test__recreate_access_token__success_case(
    ...
    response = await api_client.post("/api/v1/auth/access", headers=recreate_access_token_headers)
...
@pytest.mark.asyncio
async def test__recreate_access_token__jwt_session_is_denied(
    async_db_session: AsyncSession,
    api_client: AsyncClient,
) -> None:
    user_raw_password = get_random_str()
...
@pytest.mark.asyncio
async def test__recreate_access_token__not_exists_jwt_session(
    async_db_session: AsyncSession,
    api_client: AsyncClient,
) -> None:
    user_raw_password = get_random_str()
...
@pytest.mark.asyncio
async def test__recreate_access_token__expired_refresh_token(
    async_db_session: AsyncSession,
    api_client: AsyncClient,
) -> None:
    user = await UserFactory.create(
...
@pytest.mark.asyncio
async def test__recreate_access_token__without_refresh_token(
    async_db_session: AsyncSession,
    api_client: AsyncClient,
) -> None:
    response = await api_client.post("/api/v1/auth/access")
    response_data = response.json()
...
@pytest.mark.asyncio
async def test__recreate_access_token__invalid_refresh_token(
    async_db_session: AsyncSession,
    api_client: AsyncClient,
) -> None:
    recreate_access_token_headers = {
